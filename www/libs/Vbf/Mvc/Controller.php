...
abstract class Vbf_Mvc_Controller
{
  ...
  public  $errors        = array();
  public  $errorsField   = array();
  public  $errorsCustom  = array();
  
  protected $globalViews = 'main';
  protected function getGlobalView()              { return $this->globalViews; }
  public function setGlobalView($globalViews)     { $this->globalViews = $globalViews; }
  ...
  private function getViewFilePath($view) {
    if ($view == '') {
      $view = $this->methodName;
    }
    $path = $this->modulePath;
    if ( (count($view) > 0) && ($view[0] == '/') ) {
      $view = substr($view, 1);
  ...
  private function getView($view) {
    ...
    if (!file_exists($path)) {
      throw new Vbf_Mvc_Exception404("File not found: $path");
    }
