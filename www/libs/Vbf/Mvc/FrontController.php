...
class Vbf_Mvc_FrontController
{
   ...
   * Get the current URI.
   */
  public function getCurrentUri() { return $this->parsedUri['path']; }
  ...
  public function formatUri($uri, $urlencode = false) {
    $uri = $this->formatSkinUri($uri);
    $uri = $this->formatJSUri($uri);
    if ($urlencode) $uri = urlencode($uri);
    return $uri;
  }

  /**
   * Replace token skin from URI
   */
  public function formatSkinUri($uri) {
    global $g_options;
    $search  = array();
    $replace = array();
    $default = "";
    ...
    if (array_key_exists('skin', $g_options)) {
      if (array_key_exists('default', $g_options['skin'])) $default = $g_options['skin']['default'];
      if (array_key_exists('url',     $g_options['skin'])) $url     = $g_options['skin']['url'];

      if (array_key_exists('mask', $g_options['skin'])) {
    	if (!is_array($g_options['skin']['mask'])) $g_options['skin']['mask'] = array($g_options['skin']['mask']);
    
    	foreach ($g_options['skin']['mask'] as $skin) {
    	  if (!array_key_exists('token', $skin)) throw new Exception('No token skin found.');
    	  $search[]  = '/\/?'.$skin['token'].'\/?/';
    	  $replace[] = $url.'/'.(array_key_exists('name', $skin) ? $skin['name'] : $default).'/';
    	}
      }
    }
    $search[]  = '/\/?#[A-Z_-]+#\/?/';
    $replace[] = $url.'/'.$default.'/';
    return preg_replace($search, $replace, $uri);
  }

  /**
   * Minify js uri
   */
  public function formatJSUri($uri) {
    global $g_options;

    $info = pathinfo($uri);
    if (array_key_exists('extension', $info) && $info['extension'] == 'js' && array_key_exists('javascript', $g_options) &&
          array_key_exists('debug', $g_options['javascript']) && $g_options['javascript']['debug'] == FALSE) {
      $url  = $info['dirname'].'/'.$info['filname'].'.min.js';
      if (file_exists($url)) {
	    $uri = $url;
      }
    }
  ...
  private static $extensionContentType = array(
   ...
   'xml'  => array('application/xml', 'text/xml'), // RFC3023
  ...
  public function run() {
    ...
    try {
      ...
      $this->controllerInstance->initialize(
        $this,
  ...
  private function initializeExtension() {
    ...
    $this->contentType = array_pop(self::$extensionContentType[$this->extension]);
  }
